...
<img src="images/Java高级应用-第10章.png" alt="第3阶段：Java高级应用-第10章" style="zoom: 50%;" />
...
3. 调用线程对象的start()方法来**启动该线程**, 该方法继承自父类
4. 调用的时候要用start而不是run，只用run只是单线程
...
2. 创建Runnable实现类的实例，并以此实例作为Thread的**target参数来创建Thread对象**，该Thread对象才是真正的线程对象。
