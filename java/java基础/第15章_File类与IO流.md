...
- 想要在Java程序中表示一个真实存在的文件或目录，那么必须有一个File对象，但是**Java程序中的一个File对象，可能没有一个真实存在的文件或目录**。
...
* `public String getAbsolutePath()`：获取绝对路径， 返回值为String
* `public File getAbsoluteFile()`：获取绝对路径表示的文件,返回值为File
...
public class Employee implements Serializable {
    //static final long serialVersionUID = 23234234234L; 不定义会自动生成，但是
    //随着类的改变会发生改变。比如类新增了属性或方法，这样之后反序列化失败
