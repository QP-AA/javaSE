...
```
两个  // value数组一个(堆空间new的)， value指向的字符串一个(字符串常量池)
```
...
说明：实际上原来的“a”字符串对象已经丢弃了，现在在**堆空间**中产生了一个字符串s1+"b"（也就是"ab")。如果多次执行这些改变串内容的操作，会导致大量副本字符串对象存留在内存中，降低效率。如果这样的操作放到循环中，会极大影响程序的性能。
...
@Test
public void test02(){
	final String s1 = "hello";  // final修饰为常量
...
（10）`String trim()`：去掉字符串前后空白符  只能去掉前后， 中间的去掉不了
